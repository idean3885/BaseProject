...
public class MemberControllerTest {
    ...
    public void select_return_MemberSelectRspDto() throws Exception {
        ...
        // when
        mockMvc.perform(get("/member/" + ANY_MBR_ID))
                // then
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
    ...
    public void select_with_invalid_mbrId_type_throw_MethodArgumentTypeMismatchException() throws Exception {
        ...
        // when
        mockMvc.perform(get("/member/" + WRONG_MBR_ID))
                // then
                .andExpect(status().isBadRequest())
                .andExpect(result -> assertThat(TestAdviceUtil.getApiResultExceptionClass(result))
    ...
    public void update_return_MemberUpdateRspDto() throws Exception {
        ...
        var reqDto = MemberUpdateReqDto.builder()
                ...
                .build();
        var reqJson = objectMapper.writerWithDefaultPrettyPrinter().writeValueAsString(reqDto);
        var updatedMember = Member.builder()
                .mbrPswd(reqDto.getMbrPswd())
                .mbrTelno(reqDto.getMbrTelno())
                .mbrRoadNmAddr(reqDto.getMbrRoadNmAddr())
                .mbrDaddr(reqDto.getMbrDaddr())
        ...
        // when
        mockMvc.perform(post("/member/" + ANY_MBR_ID)
                        .contentType(MediaType.APPLICATION_JSON_VALUE)
                        .content(reqJson)
                )
                // then
    ...
    public void delete_return_MemberDeleteRspDto() throws Exception {
        ...
        var delelteMember = Member.builder()
                .mbrEml("valid@email.com")
                .mbrPswd("pswd")
                .mbrNm("mbrNm")
                .mbrTelno("01234567890")
        ...
        // when
        mockMvc.perform(delete("/member/" + ANY_MBR_ID))
                // then
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
